...
def rnn_step_backward(dnext_h, cache):
    ...
    ##############################################################################
    # *****START OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
    next_h,x,prev_h,Wx,Wh = cache #b는 굳이 cache에 담을 필요 x
    dlocal = dnext_h*(1-np.square(next_h))
    dx = np.dot(dlocal,Wx.T)
    dprev_h = np.dot(dlocal,Wh.T)
    dWx = np.dot(x.T,dlocal)
    dWh = np.dot(prev_h.T,dlocal)
    db = np.sum(dlocal,axis=0) #N축으로의 sum->H*1
...
def rnn_forward(x, h0, Wx, Wh, b):
    ...
    - cache: Values needed in the backward pass->next_h,x,prev_h,Wx,Wh
    """
    h, cache = None, None
    ##############################################################################
    ...
    N,T,D = x.shape
    H = h0.shape[1]
    cache = [] #->rnn_step_forward에서 cache는 특정 timestep에 대한 cache만 저장
    h = np.zeros((N,T,H))
    #Process by time series
    prev_h = h0
    for t in range(T):
      xt = x[:,t,:]
      next_h,ct = rnn_step_forward(xt, prev_h, Wx, Wh, b)#(x, prev_h, Wx, Wh, b
      cache.append(ct) #next_h,x,prev_h,Wx,Wh->N,T,D
      h[:,t,:] = next_h
      prev_h = next_h
    # *****END OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
    ##############################################################################
...
def rnn_backward(dh, cache):
    ...
    for t in range(T-1,-1,-1):
      dh_t = dh[:,t,:]+dprev_ht
      dx_t, dprev_ht, dWx_t, dWh_t, db_t = rnn_step_backward(dh_t,cache[t])
      dx[:,t,:] = dx_t
      dWx+=dWx_t
      dWh+=dWh_t
      db+=db_t
...
def word_embedding_backward(dout, cache):
    ...
    for i in range(N):
      grads = dout[i,:,:]
      indices = x[i,:] #->c.f) v1,v3,v4
      np.add.at(dW,indices,grads)
...
def lstm_step_forward(x, prev_h, prev_c, Wx, Wh, b):
    ...
    N,H = x.shape[0],prev_h.shape[1]
    gates = np.dot(x,Wx)+np.dot(prev_h,Wh)+b #(N,4H)-> 각 data마다 dim이 4H인 gate 생성
    i_b,f_b,o_b,g_b = np.split(gates,4,axis = 1)
    i = sigmoid(i_b)
    f = sigmoid(f_b)
    o = sigmoid(o_b)
    g = np.tanh(g_b)
    before_gates = (i_b,f_b,o_b,g_b)
    gates = (i,f,o,g)
    next_c = f*prev_c+i*g
    tanh_c = np.tanh(next_c)
    next_h = o*tanh_c
    cache = (x,prev_h,prev_c,Wx,Wh,b,tanh_c,before_gates,gates)
...
def lstm_step_backward(dnext_h, dnext_c, cache):
    ...
    #############################################################################
    # *****START OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
    x,prev_h,prev_c,Wx,Wh,b,tanh_c,before_gates,gates = cache
    i_b,f_b,o_b,g_b = before_gates
    i,f,o,g = gates
    dnext_c += dnext_h*o*(1-np.square(tanh_c))
    dprev_c = dnext_c*f
    df = dnext_c*prev_c #(N,H)
    do = dnext_h*tanh_c #(N,H)
    di = dnext_c*g #(N,H)
    dg = dnext_c*i #(N,H)
    di_b = i*(1-i)*di 
    df_b = f*(1-f)*df
    do_b = o*(1-o)*do
    dg_b = (1-g**2)*dg
    dgates = np.concatenate((di_b, df_b, do_b, dg_b), axis=1)
    
    dx = np.dot(dgates,Wx.T)
    dprev_h = np.dot(dgates,Wh.T)
    dWx = np.dot(x.T,dgates)
    dWh = np.dot(prev_h.T,dgates)
...
def lstm_forward(x, h0, Wx, Wh, b):
    ...
    for t in range(T):
      xt = x[:,t,:]
      next_h, next_c, ct = lstm_step_forward(xt, prev_h, prev_c, Wx, Wh, b)
      cache.append(ct)
      h[:,t,:] = next_h
      prev_h = next_h
      prev_c = next_c
...
def lstm_backward(dh, cache):
    ...
    for t in range(T-1,-1,-1):
      dh_t = dh[:,t,:]+dprev_ht
      dx_t, dprev_ht, dprev_ct,dWx_t, dWh_t, db_t = lstm_step_backward(dh_t,dprev_ct,cache[t])
      dx[:,t,:] = dx_t
      dWx+=dWx_t
      dWh+=dWh_t
      db+=db_t
