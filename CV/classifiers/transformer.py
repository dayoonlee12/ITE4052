...
class CaptioningTransformer(nn.Module):
    ...
    def __init__(self, word_to_idx, input_dim, wordvec_dim, num_heads=4,
        ...
        #embedding을 통해 vocab_size=3개 짜리였을 때 각각의 vocab은 30개의 vector로 embed됨
        ...
        self.apply(self._init_weights) #decod
    ...
    def forward(self, features, captions):
        ...
        #1) embed caption -> add positional encoding-> project image features into the same dim
        #print(captions.shape)
        x = self.embedding(captions) #(N,T)->(N,T,W)
        x = self.positional_encoding(x) 
        feats = self.visual_projection(features) #(N,D)->(N,W)

        #2
        tgt_mask = torch.tril(torch.ones((T, T), device=captions.device))
        out = self.transformer(x, feats.unsqueeze(1), tgt_mask=tgt_mask)
...
class TransformerDecoderLayer(nn.Module):
    ...
    def forward(self, tgt, memory, tgt_mask=None):
        ...
        # layer norm).
        # print("now process to :MultiheadAttention(input_dim,num_heads,dropout):")
        # print(f"query,key,value:{tgt.shape}")
        # print(f"attn_mask:{tgt_mask.shape}")
        tgt2 = self.self_attn(query=tgt, key=tgt, value=tgt, attn_mask=tgt_mask)
        # print(tgt2.shape,attn_map1.shape)
